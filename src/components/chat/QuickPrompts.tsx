const PROMPTS: Record<string, { label: string; prompt: string }[]> = {
  all: [
    { label: "🏋️ Build a workout plan", prompt: "Create a 4-day workout plan for building muscle as a beginner." },
  ...
  form: [
    ...
    { label: "⚠️ Common mistakes", prompt: "What are the most common form mistakes beginners make in the gym?" },
  ],
};
      ...
      <div className="grid grid-cols-2 gap-2 max-w-2xl mx-auto">
        {prompts.map((p) => (
          <button
            ...
            className="text-left text-sm bg-muted hover:bg-secondary border border-border hover:border-primary/50 text-foreground rounded-xl px-3 py-2 transition-all duration-200 hover:text-primary leading-snug"
          >
            {p.label}
          </button>
        ))}
